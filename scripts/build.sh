...
# `linguas: cd po && make ... linguas` — the po/ sub-Makefile is
# generated by gettext's `linguas.m4`, and the `linguas` target
# fails when --without-gettext is set (no po/Makefile is created).
# Patch the generated Makefile to make `linguas` a no-op. The
# translations are Dutch-only and not required for the dist binary.
echo "==> patch Makefile: no-op the linguas target (no gettext)"
( cd "$DWDIFF_SRC" && \
  sed -i.bak 's/^linguas:$/linguas: ;/' Makefile && \
  rm -f Makefile.bak )
